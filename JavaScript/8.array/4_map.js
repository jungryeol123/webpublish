...
nArray.forEach((element,i) => console.log(`nArray[${i}] = ${element}`));

//nArray 요소의 소수점 절삭 후 데이터 출력 : map()
let nArray2 = nArray.map((item) => {return Math.floor(item);});     // [1, ...] , {}를 생략안할 경우 return 필수
nArray2.forEach(item => console.log(item));

//numbers 요소에 100을 곱하여 새로운 객체를 생성
let numbers2 = numbers.map((item) => item*100);
numbers2.forEach((item) => console.log(item));

//objects 요소 중 이름이 이순신인 객체를 새로운 배열객체로 생성
